...
### Sesión 5 — CRM: B-15 CAPI Enhancement (feature)  ✅ HECHA + DESPLEGADA + QA PASS (evento ACEPTADO por Meta, status=sent)
...
**✅ RESUELTO (fix de config, NO código) — por esto el CAPI llevaba MUERTO en prod:** el `agency_config.meta_pixel_id`
estaba en `443666763866610744` (**inválido** → Meta 400). Diagnóstico con Ivan en Events Manager: ese valor era un typo
del **App ID** `4436676386610744`, que además resultó ser un **dataset de APP** (no un Pixel web) → tipo equivocado para
eventos de servidor ("does not support this operation"). El **Pixel web correcto = `1417942562598927` ("RE/MAX Advance -
R.D.")**, el dataset que Meta marca con el banner "Conectar CRM / Termina de configurar API de conversiones para CRM".
Fix aplicado vía MCP: `UPDATE agency_config SET value='1417942562598927' WHERE key='meta_pixel_id'` (sin redeploy —
`resolvePixelId` ya lo lee; `META_PIXEL_ID` no está en Vercel, por eso usa agency_config). **QA final:** sembré un
evento de verificación en `capi_outbox`; el cron lo despachó y **Meta lo ACEPTÓ (`status='sent'`, 2xx)** con el
`META_ACCESS_TOKEN` existente (el token SÍ tiene acceso a ese dataset — no hizo falta token dedicado). Evento de prueba
limpiado. **El CAPI del CRM quedó VIVO.** (Nota: existe otro Pixel "Remax advance rd's Pixel" `655947334056023`; si las
campañas de leads llegaran a optimizar contra ese, cambiar el valor de agency_config — es 1 UPDATE, sin deploy.)
