-- 0022_unify_commission_views.sql
-- Unify commission calculation across the app. Commission is MANUAL per deal
-- (deals.commission_percentage, editable). Remove the hardcoded 3% fallback that
-- diverged from the TS screens (Reports/campaigns ignored the % entirely).
-- New unified rule everywhere: commission_value ?? (deal_value * commission_percentage / 100),
-- with an unset percentage yielding 0 (never a magic number). Mirrors lib/commission.ts.
-- Only the inner COALESCE(..., 3::numeric) -> COALESCE(..., 0::numeric) changes; every
-- other line is copied byte-for-byte from 0012_views_exclude_new_stage.sql. Since all
-- existing deals have commission_percentage = 3.00, no existing revenue number changes.

CREATE OR REPLACE VIEW public.agent_monthly_kpis AS
 WITH month_bounds AS (
         ...
         SELECT d.agent_id,
            ...
            sum(COALESCE(d.commission_value, d.deal_value * COALESCE(d.commission_percentage, 0::numeric) / 100.0)) AS total_revenue
           FROM deals d,
            month_bounds mb
...
CREATE OR REPLACE VIEW public.agent_historical_kpis AS
 WITH closed_by_month AS (
         ...
         SELECT deals.agent_id,
           ...
           FROM deals
          WHERE deals.created_at >= (date_trunc('month'::text, now()) - '1 year 11 mons'::interval) AND deals.stage <> 'nuevo_sin_contactar'::deal_stage
